...
function renderMultiSeriesHorizontalBarChat(
  elem,
  unsortedData,
  width,
  height,
    ...
    .call(function (t) {
      t.each(function (d) {
        ...
        if (name.length > MAX_LINE_SIZE && name.split(" ").length > 1) {
          ...
          let list = name.split(" ");
          let num_lines = Math.min(
            1 + Math.floor(name.length / MAX_LINE_SIZE),
            list.length
          );
          let cur = 0;
          let split_name = [];
          for (let idx = 0; idx < num_lines; idx++) {
            ...
            while (
              cur < list.length - 1 &&
              count + list[cur + 1].length + 1 <= MAX_LINE_SIZE
            ) {
...
function renderGroupedBarChart(elem, data, width, height, keys, options) {
  ...
  const x0 = d3.scaleBand().rangeRound([0, graphWidth]).paddingInner(0.1);
  ...
  // color
  const colorScale = d3.scaleOrdinal().range(termColors);
  ...
  g.append("g")
    ...
    .selectAll("rect")
    .data(function (d) {
      return graphKeys.map(function (key) {
...
export { renderMultiSeriesHorizontalBarChat, renderGroupedBarChart };
